...
3. Compare old vs new using `compareKeys` / `compareCertificates` (subject-map equality for certs, serialized PEM equality for keys).
4. Write to disk only if changed (or if no existing file), using `saveToFile` / `saveCertKeyChainToFile`.
...
|---|---|---|
| `privatekey` | `/privatekeys/` | `<name>_key.pem` |
| `certificate` | `/certificates/` | `<name>_certchain.pem` |
| `privatecertchain` | `/privatecertchains/` | `<name>_keycertchain.pem` |
| `pfx` | `/pfx/` | `<name>.p12` |
...
Two-level YAML: `global` block (defaults for all certs) and a `certificates` list. Per-certificate values override globals. `refreshPeriod` accepts any string parseable by `fortio.org/duration` (e.g. `1w2d3h`). `permissions` is an octal integer (e.g. `0640`). The only required field is `global.certWardenURL`.
